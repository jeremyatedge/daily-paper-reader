...
tags: ["query:aiot"]
score: 8.0
evidence: 用于AI眼镜节能的语义代理通信
tldr: 针对智能眼镜在将视觉数据传输至云端大模型时面临的带宽和能耗限制，本文提出一种意图感知语义通信框架。该框架通过云端模型推断用户意图，指导眼镜端自适应地保留文本、布局或物体语义进行传输。研究表明，该方法在显著降低带宽需求的同时保持了任务性能，为高效可穿戴AI系统提供了新思路。
selection_source: carryover_cache
motivation: 智能眼镜持续向云端传输原始视觉数据受限于无线带宽和设备能耗，难以支持实时AI辅助。
method: 提出一种意图感知框架，由服务器端推断用户意图并指导眼镜端进行针对性的语义预处理与传输。
result: "实验表明该方法在保持任务性能的前提下，可减少超过50%的带宽消耗，且在低信噪比环境下表现稳健。"
conclusion: 将通信资源与用户意图对齐是构建高效、鲁棒的可穿戴AI智能体系统的关键。
---
