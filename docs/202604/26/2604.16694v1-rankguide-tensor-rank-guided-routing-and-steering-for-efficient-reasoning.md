...
title_zh: RankGuide：基于张量秩引导的路由与转向，实现高效推理
authors: "Jiayi Tian, Yupeng Su, Ryan Solgi, Souvik Kundu, Zheng Zhang"
date: 2026-04-17
pdf: "https://arxiv.org/pdf/2604.16694v1"
tags: ["query:aiot"]
score: 6.0
evidence: 小型和大型推理模型之间的协作以提高效率
tldr: 针对大推理模型（LRM）推理延迟高的问题，本文提出RankGuide框架，通过小推理模型（SRM）与LRM的协作来提升效率。研究者识别了SRM的三种失败模式，并利用连续隐藏状态的张量秩信号进行路由决策（何时调用LRM）和引导向量提取（优化SRM生成）。该方法在保持高准确率的同时，显著降低了推理延迟，为高效推理提供了新思路。
selection_source: fresh_fetch
motivation: 旨在解决小推理模型在协作推理中容易出现失败且难以高效检测和修复的问题。
method: 提出RankGuide框架，利用张量秩信号进行路由选择以识别SRM失败，并结合张量秩过滤的引导向量来优化SRM的推理轨迹。
result: 在多个基准测试中，RankGuide相比LRM实现了高达1.75倍的延迟降低，且准确率优于现有协作方法。
conclusion: 通过张量秩信号引导的路由和转向机制，可以有效提升大小模型协作推理的效率与性能。
---

## 摘要
大型推理模型 (LRMs) 通过生成显式的多步思维链 (CoT) 推理来增强问题解决能力；然而，它们会产生巨大的推理延迟和计算开销。为了缓解这一问题，最近的研究探索了模型协作范式，即由小型推理模型 (SRMs) 生成中间推理步骤，以实现更好的准确率-延迟权衡。尽管取得了进展，但在协作系统中有效且高效地检测并缓解 SRM 的失败仍然是一个关键挑战。为了解决这一问题，我们从生成的文本和隐藏状态空间两个维度分析了 SRM 的推理过程，并识别出三种失败模式：过度自信 (overconfidence)、不确定性 (uncertainty) 和过度重新验证 (heavy revalidation)。基于这些见解，我们提出了 RankGuide，这是一个通过张量秩引导的路由和转向来提高 SRM-LRM 协作效率和有效性的框架。具体而言，RankGuide 利用一种包含源自连续隐藏状态的张量秩信号的路由信号，来检测 SRM 何时可能失败，并选择性地调用 LRM。此外，我们引入了一种基于张量秩过滤的转向向量提取方法，用于调节 SRM 的推理轨迹，从而提高其生成质量。通过利用张量秩信号改进路由和转向，RankGuide 使 SRM-LRM 协作系统能够以更少的步骤和更高的准确率实现更高效的推理。在多个推理基准测试上的实验表明，与 LRM 相比，RankGuide 在降低延迟方面具有显著成效（最高可达 1.75 倍），同时保持了与现有方法相比具有竞争力的准确率。
