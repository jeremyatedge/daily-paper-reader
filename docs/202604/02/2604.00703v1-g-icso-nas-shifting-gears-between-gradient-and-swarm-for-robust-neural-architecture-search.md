...
## 摘要
神经架构搜索（NAS）已成为自动机器学习中的关键技术。基于进化算法（EA）的方法表现出优异的搜索质量，但面临极高的计算成本；而像 DARTS 这样的基于梯度的方法虽然效率高，但容易出现过早收敛和性能崩溃。为了弥补这一差距，我们提出了 G-ICSO-NAS，这是一个实现三阶段优化策略的混合框架。预热阶段通过可微分方法预训练超网络权重（$w$），同时保持架构参数（$α$）冻结。探索阶段采用混合协同优化机制：具有多样性感知适应度的改进竞争群优化器（ICSO）在架构空间中导航以更新 $α$，同时梯度下降同步更新 $w$。稳定阶段采用带有早停机制的细粒度梯度搜索，以收敛至最优架构。通过协同 ICSO 的全局导航能力与可微分方法的效率，G-ICSO-NAS 以极低的成本实现了卓越的性能。在 DARTS 搜索空间下，仅需 0.15 GPU 天的计算预算，即可在 CIFAR-10 上达到 97.46% 的准确率。该方法还表现出强大的迁移潜力，在 CIFAR-100 和 ImageNet 上的准确率分别为 83.1% 和 75.02%。此外，在 NAS-Bench-201 基准测试中，G-ICSO-NAS 在所有评估数据集上均展现出最先进的性能。
