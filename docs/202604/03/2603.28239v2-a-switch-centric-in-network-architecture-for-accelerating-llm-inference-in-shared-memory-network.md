...
## 摘要
以 NVLink Sharp (NVLS) 为代表的网络内计算技术，通过将 All-Reduce 等集合操作卸载到交换机，为解决大语言模型（LLM）推理中的通信瓶颈提供了一种极具前景的方法。然而，NVLS 以加速器为中心的架构存在两个根本局限性：1) 它依赖 GPU 的加载指令来触发归约操作，这意味着在交换机中完成归约的数据必须额外传输回发起请求的 GPU，而不能直接进行广播，从而引入了不必要的通信开销；2) 受限于架构约束，NVLS 无法卸载不能分解为内存语义指令的算子，例如本研究提出的网络内量化（INQ）。因此，NVLS 中的 All-Reduce 必须以 FP16/BF16 精度运行，导致了大量的带宽浪费。为解决这些局限性，我们提出了 SCIN，这是首个针对 AI 加速器共享内存网络的以交换机为中心的网络内架构，能够实现低延迟且高带宽的 All-Reduce。具体而言，我们引入了一种交换机内加速器（ISA），它能够发起用于网络内处理的内存语义操作，并配合一种协同设计的通信织网，从而产生极低的协议开销。通过消除冗余的数据移动，SCIN 实现了比 NVLS 更低的 All-Reduce 延迟。此外，通过在 ISA 中集成量化模块，SCIN 实现了 All-Reduce 的网络内量化（INQ），将其精度降低至 8 位，在精度损失可忽略不计的情况下使带宽几乎翻倍。我们还在多 FPGA 系统上展示了 SCIN 的原型，以证明其可行性和有效性。实验结果表明，我们的设计在处理小消息和大消息时，分别将 All-Reduce 加速了高达 8.7 倍和 3.8 倍，使 LLaMA-2 模型的首字生成时间（TTFT）提升了 1.74 倍，逐字生成时间（TPOT）提升了 1.34 倍。
