...
tags: ["query:aiot"]
score: 6.0
evidence: 用于具身智能的轻量级视觉-语言-动作模型
tldr: "针对具身智能数据生成中的探索受限和模型崩溃问题，本文提出Seed2Scale自进化数据引擎。该引擎通过“小模型采集、大模型评估、目标模型学习”的异构协同机制，仅需4个种子演示即可在并行环境中高效生成高质量数据。通过引入轻量级SuperTiny模型进行探索和预训练VLM进行质量评分，有效解决了自迭代过程中的性能退化，实现了131.2%的性能提升，为通用具身智能的规模化发展提供了低成本路径。"
selection_source: fresh_fetch
motivation: 现有的具身智能数据生成方法面临探索受限、具身间隙和低信噪比导致的自迭代性能退化问题。
method: 提出一种结合轻量级采集模型SuperTiny、大模型验证器和目标模型学习的异构协同自进化数据引擎。
result: "实验显示该引擎具有显著的扩展潜力，使目标模型的成功率提升了131.2%，并有效缓解了模型崩溃。"
conclusion: Seed2Scale为大规模开发通用具身智能提供了一种可扩展且极具成本效益的数据生成方案。
---
