...
tags: ["query:aiot"]
score: 8.0
evidence: 无线供电移动边缘计算的节能在线调度
tldr: 本文研究无线供电移动边缘计算（WP-MEC）网络中的能效在线调度问题。针对无线能量传输与计算卸载之间的资源竞争，提出一种基于Lyapunov优化的在线框架，将随机问题转化为每时隙确定性问题。通过引入边际能效概念和松弛调整法，并结合指派问题转化，有效解决了非凸优化难题。该方案在保证系统稳定性的同时，实现了时延与能耗的平衡，并通过仿真验证了其在多设备多接入点场景下的高效性与鲁棒性。
selection_source: fresh_fetch
motivation: 旨在解决无线供电移动边缘计算网络中，无线能量传输与计算卸载之间复杂的资源竞争及能效优化挑战。
method: 采用Lyapunov优化框架将长期随机问题转化为每时隙优化，并利用边际能效分析和指派问题转化来高效求解非凸子问题。
result: 理论证明并仿真验证了该算法能有效平衡系统时延与能耗，并提供了明确的性能界限。
conclusion: 该在线调度算法为WP-MEC系统提供了一种低复杂度且鲁棒的能效优化方案。
---

## 摘要
无线供电移动边缘计算（WP-MEC）将移动边缘计算（MEC）与无线电能传输（WPT）相结合，旨在同时延长无线设备（WD）的使用寿命并增强其计算能力。在 WP-MEC 系统中，WPT 与计算卸载竞争有限的无线资源，这使得它们的联合调度具有极大的挑战性。本文研究了具有多个无线设备和多个接入点（AP）的 WP-MEC 网络中的能效在线调度问题。基于 Lyapunov 优化，我们开发了一个在线优化框架，将原始随机问题转化为确定性的逐时隙优化问题。为了降低计算复杂度，我们引入了边际能效的概念并推导了相关的最优性条件，据此提出了一种“先松弛后调整”的方法来高效获取可行解。针对由此产生的非凸计算卸载子问题，我们分析了其最优解的结构特性，并将其转化为可以高效求解的指派问题。我们进一步为逐时隙和长期解提供了理论性能保证，确立了延迟与能耗之间的基本权衡关系。为了提高实际性能，我们引入了额外的机制来平衡不同队列的规模，并在不增加能耗的情况下降低延迟。广泛的仿真结果证明了所提算法在各种系统设置下的有效性和鲁棒性。
