...
tldr: 针对视觉-语言-动作（VLA）模型中流匹配动作头推理延迟高的问题，本文提出 ProbeFlow 框架。该方法无需训练，通过计算初始与前瞻速度向量的余弦相似度来评估轨迹几何复杂度，从而动态调整 ODE 求解步数。实验表明，ProbeFlow 在保持操作成功率的同时，显著提升了动作解码速度和系统端到端响应能力，为低延迟连续生成策略提供了实用方案。
selection_source: fresh_fetch
motivation: 流匹配动作头需要多步迭代求解 ODE，产生的推理延迟限制了机器人在复杂任务中的实时响应能力。
method: 通过评估初始与前瞻速度向量的余弦相似度来衡量轨迹几何复杂度，从而动态调度积分步数以减少冗余计算。
result: 在 MetaWorld 基准测试中实现动作解码加速 14.8 倍，端到端延迟降低 2.8 倍，且未损失操作成功率。
conclusion: ProbeFlow 是一种无需训练且高效的自适应推理方案，成功解决了 VLA 模型动作解码的瓶颈，确保了物理执行的稳定性。
---
