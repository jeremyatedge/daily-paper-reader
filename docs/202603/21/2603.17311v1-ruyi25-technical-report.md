...
conclusion: Ruyi2.5通过统一的训练架构和创新的优化算法，实现了高性能、高效率且具备隐私保护能力的多模态模型部署。
---

## 摘要
我们介绍了 Ruyi2.5，这是一个基于 AI Flow 框架构建的多模态家族模型。Ruyi2.5 将 Ruyi2 的“一次训练，多次部署”范式扩展到多模态领域，构建了一个共享骨干网络架构，在单一统一流水线中协同训练不同规模的模型，确保了所有部署层级间的语义一致性。基于 Ruyi2.5，我们开发了 Ruyi2.5-Camera 模型作为一种隐私保护摄像头服务系统，该系统将 Ruyi2.5-Camera 实例化为一个两阶段识别流水线：边缘模型应用信息瓶颈引导的不可逆特征映射，在源头对原始帧进行去标识化处理，而云端模型则执行深度行为推理。为了加速强化学习微调，我们进一步提出了二值前缀策略优化（BPPO），该方法通过二值响应选择减少样本冗余，并将梯度更新集中在响应前缀上，相比 GRPO 实现了 2 到 3 倍的训练加速。实验表明，Ruyi2.5 在通用多模态基准测试上与 Qwen3-VL 表现相当，而 Ruyi2.5-Camera 在隐私受限的监控任务中显著优于 Qwen3-VL。
