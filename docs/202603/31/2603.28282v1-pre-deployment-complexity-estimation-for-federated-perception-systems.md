...
tldr: 针对边缘AI系统中联邦学习任务在部署前难以预估难度的问题，本文提出了一种与分类器无关的预部署复杂度评估框架。该框架通过综合建模数据的内在属性（如维度、稀疏性、异构性）以及分布式环境中的客户端特征，生成一个复杂度指标。实验证明，该指标能有效预测联邦学习的最终准确率和通信成本，为资源规划和可行性评估提供了重要参考。
selection_source: fresh_fetch
motivation: 开发者在联邦学习训练开始前，缺乏评估任务达成目标准确率的难度及所需通信成本的有效手段。
method: 提出一种整合数据集属性与客户端分布特征的复杂度度量指标，用于在模型部署前量化感知任务的学习难度。
result: 实验显示该指标与模型性能及通信开销具有强相关性，能准确反映不同联邦配置下的学习难度。
conclusion: 预部署复杂度评估是边缘感知系统进行资源分配、数据集质量检查和方案可行性验证的有效工具。
---
