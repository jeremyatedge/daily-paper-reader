...
## 摘要
现代云端推理产生了一个双向隐私问题：用户向提供商泄露敏感输入，而提供商必须在可能存在泄露风险的执行环境中运行专有模型权重。全同态加密（FHE）提供了密码学保障，但对于现代架构而言，其开销仍然高得令人望而却步。我们认为，取得进展需要进行协同设计，即针对推理电路的静态结构专门化 FHE 方案/编译器，同时约束推理架构以减少主要的同态成本驱动因素。我们概述了一个“中途相会”的议程，并确定了两个维度上的具体优化目标。
