...
tags: ["query:aiot"]
score: 7.0
evidence: 在严格资源约束下运行的物理AI智能体
tldr: 本文从工程视角探讨了主动推理（AIF）在物理AI智能体中的应用。针对机器人等具身系统在资源受限环境下的挑战，研究基于自由能原理，将感知、学习、规划与控制统一在最小化变分自由能的框架下。通过因子图上的响应式消息传递实现局部并行计算，使系统具备异步处理、资源自适应及多尺度分层架构的能力，为构建高效、鲁棒的物理智能体提供了理论与架构支撑。
selection_source: fresh_fetch
motivation: 物理AI智能体在开放、资源受限的现实环境中表现远逊于生物体，亟需一种能统一感知与行动且适应物理约束的理论框架。
method: 基于自由能原理，利用因子图上的响应式消息传递将变分自由能最小化转化为局部、并行的事件驱动计算。
result: 该架构实现了感知、学习与控制的统一，支持在资源波动下的优雅降级，并能通过粗粒化耦合构建同构的分层智能体系统。
conclusion: 主动推理为物理AI提供了一个原则性的工程基础，其消息传递机制在处理实时性、异步性和可扩展性方面具有显著优势。
---
