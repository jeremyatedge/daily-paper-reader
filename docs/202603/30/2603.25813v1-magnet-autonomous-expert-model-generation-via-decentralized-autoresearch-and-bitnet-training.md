---
title: "MAGNET: Autonomous Expert Model Generation via Decentralized Autoresearch and BitNet Training"
title_zh: MAGNET：通过去中心化自动研究与 BitNet 训练实现自主专家模型生成
authors: "Yongwan Kim, Sungchul Park"
date: 2026-03-26
pdf: "https://arxiv.org/pdf/2603.25813v1"
tags: ["query:aiot"]
score: 6.0
evidence: 专家模型的CPU原生推理
tldr: MAGNET是一个去中心化的自主专家模型生成系统，旨在普通硬件上实现模型的自动研发、训练与部署。它集成了自动机器学习研究流水线、BitNet三值化训练（支持CPU推理）、DiLoCo分布式模型合并以及区块链贡献追踪。该系统通过自动化迭代显著提升了视频安全分类和加密货币预测等特定领域任务的性能，为低成本、去中心化的AI开发提供了新范式。
selection_source: fresh_fetch
motivation: 旨在降低专家级语言模型的开发门槛，解决对高性能GPU的依赖以及模型研发过程中的人工干预问题。
method: 结合了自动化的ML研究流水线、BitNet三值化量化技术、DiLoCo分布式合并算法以及基于区块链的贡献追踪机制。
result: "在视频安全分类中准确率提升至0.9851，加密货币预测命中率提升至54.9%，且BitNet超参数优化使验证损失降低了16.7%。"
conclusion: MAGNET证明了通过去中心化和自动化流程，可以在普通硬件上高效构建并优化高性能的领域专家模型。
---

## 摘要
我们提出了 MAGNET（模型自主增长网络），这是一个在通用硬件上实现领域专家语言模型自主生成、训练和服务的去中心化系统。MAGNET 集成了四个核心组件：(1) autoresearch，一种自主机器学习研究流水线，可实现数据集生成、超参数探索、评估和错误驱动迭代的自动化；(2) BitNet b1.58 三元训练，通过 bitnet.cpp 在无需 GPU 硬件的情况下实现 CPU 原生推理；(3) 基于 DiLoCo 的分布式合并，用于领域专家模型的高通信效率聚合；以及 (4) 在 HOOTi EVM 链上的链上贡献追踪。我们通过三个案例研究验证了 autoresearch 的有效性：视频安全分类（均衡准确率从 0.9287 提升至 0.9851）、加密货币趋势预测（命中率从 41% 提升至 54.9%）以及 BitNet 超参数优化（10 阶段扫描，验证损失降低 16.7%）。

## Abstract
We present MAGNET (Model Autonomously Growing Network), a decentralized system for autonomous generation, training, and serving of domain-expert language models across commodity hardware. MAGNET integrates four components: (1) autoresearch, an autonomous ML research pipeline that automates dataset generation, hyperparameter exploration, evaluation, and error-driven iteration; (2) BitNet b1.58 ternary training, enabling CPU-native inference via bitnet.cpp without GPU hardware; (3) DiLoCo-based distributed merging for communication-efficient aggregation of domain specialists; and (4) on-chain contribution tracking on the HOOTi EVM chain. We validate autoresearch through three case studies: video safety classification (balanced accuracy 0.9287 to 0.9851), cryptocurrency directional prediction (41% to 54.9% hit rate), and BitNet hyperparameter optimization (10-phase sweep, -16.7% validation loss).