...
tags: ["query:aiot"]
score: 7.0
evidence: 跨越边缘和终端设备的计算连续体自愈神经符号框架
tldr: 针对计算连续体中边缘设备资源受限且环境复杂的自愈难题，本文提出NeSy-Edge框架。该框架采用神经符号方法，通过边缘优先设计实现本地感知与推理，仅在诊断阶段调用云端。它将日志转化为结构化事件并构建稀疏符号因果图，结合历史知识进行根因分析。实验表明，该方法在噪声环境下具有高鲁棒性，能以较低内存占用实现高效的故障诊断与恢复。
selection_source: fresh_fetch
motivation: 现有的故障管理方法在处理计算连续体中的异构性、跨层依赖及噪声日志时，往往过于静态或资源消耗过大，难以实现及时的自愈。
method: 提出一种边缘优先的神经符号框架，通过将原始日志转化为结构化事件并构建受先验约束的稀疏符号因果图，结合历史知识进行根因分析和恢复建议。
result: "在Loghub数据集上的评估显示，该框架在极高噪声下仍能达到75%的根因分析准确率和65%的端到端准确率，且内存占用控制在1500MB以内。"
conclusion: NeSy-Edge证明了神经符号方法在边缘计算自愈场景中的有效性，实现了资源受限环境下的高鲁棒性与可信故障诊断。
---

## 摘要
现代 AI 服务的计算需求正日益从中心化云端转向跨越边缘和终端设备的计算连续体。然而，这些环境的规模、异构性以及跨层依赖性使得维持弹性变得十分困难。现有的故障管理方法通常过于静态、碎片化或过于沉重，难以支持及时的自愈，特别是在存在噪声日志和边缘资源受限的情况下。为解决这些局限性，本文提出了 NeSy-Edge，这是一个用于计算连续体中可信自愈的神经符号框架。该框架遵循边缘优先的设计，由资源受限的边缘节点执行本地感知和推理，而云端模型仅在最终诊断阶段被调用。具体而言，NeSy-Edge 将原始运行时日志转换为结构化事件表示，构建先验约束的稀疏符号因果图，并将因果证据与历史排错知识相结合，用于根因分析和恢复建议。我们在具有代表性的 Loghub 数据集上，针对多种级别的语义噪声评估了我们的工作，考虑了解析质量、因果推理、端到端诊断以及边缘侧资源使用情况。结果表明，NeSy-Edge 即使在最高噪声水平下仍保持鲁棒性，在约 1500 MB 的本地内存占用下，实现了高达 75% 的根因分析准确率和 65% 的端到端准确率。
