...
tldr: 本研究针对数据库中AI查询（如LLM驱动的SQL函数）成本高、延迟大的问题，提出了一种基于轻量级代理模型的近似方法。通过在嵌入向量上构建廉价且准确的代理模型，该方法在保持甚至提升准确率的同时，实现了语义过滤（AI.IF）和排名（AI.RANK）操作超过100倍的成本与延迟缩减。研究展示了在Google BigQuery和AlloyDB中的架构实现，证明了该方案在大规模结构化与非结构化数据融合查询中的高效性与实用性。
selection_source: fresh_fetch
motivation: 尽管LLM为数据库提供了强大的语义推理能力，但在处理大规模数据时，频繁调用LLM会导致极高的成本和无法接受的延迟。
method: 提出利用基于嵌入向量的轻量级代理模型来近似替代昂贵的LLM调用，并针对OLAP和HTAP场景设计了相应的系统架构。
result: 在包括千万级行数的亚马逊评论数据集在内的多个基准测试中，该方法实现了100倍以上的成本和延迟降低，且准确率保持稳定甚至有所提升。
conclusion: 通过轻量级代理模型进行AI查询近似，是实现低成本、高性能语义分析及数据库应用的一种极具前景的解决方案。
---
