...
motivation: 旨在解决大语言模型在数据稀缺的NPU编程领域因缺乏训练数据而导致的性能骤降和冷启动挑战。
method: 提出一种自进化的智能体框架，通过价值驱动的记忆检索机制和跨任务经验共享，实现内核的自动生成与性能迭代。
result: "在NPU基准测试中将模型正确率从11.0%大幅提升至83.0%，并相比初始草案实现了3.60倍的中位数加速。"
conclusion: 证明了通过价值引导的经验积累，通用大模型能够有效克服数据匮乏，掌握特定硬件生态下的内核合成任务。
---
